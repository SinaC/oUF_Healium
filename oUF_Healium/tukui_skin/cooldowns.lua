...
local EXPIRING_FORMAT = H.RGBToHex(1, 0, 0)..'%.1f|r' --format for timers that are soon to expire
local SECONDS_FORMAT = H.RGBToHex(1, 1, 0)..'%d|r' --format for timers that have seconds remaining
local MINUTES_FORMAT = H.RGBToHex(1, 1, 1)..'%dm|r' --format for timers that have minutes remaining
local HOURS_FORMAT = H.RGBToHex(0.4, 1, 1)..'%dh|r' --format for timers that have hours remaining
local DAYS_FORMAT = H.RGBToHex(0.4, 0.4, 1)..'%dh|r' --format for timers that have days remaining
...
local function getTimeText(s)
	...
	if s < MINUTEISH then
		local seconds = tonumber(H.Round(s))
...
local function Timer_OnSizeChanged(self, width, height)
	local fontScale = H.Round(width) / ICON_SIZE
...
local function Timer_OnUpdate(self, elapsed)
	if self.nextUpdate > 0 then
		...
		if tonumber(H.Round(remain)) > 0 then
...
hooksecurefunc(getmetatable(ActionButton1Cooldown).__index, 'SetCooldown', function(self, start, duration)
	--print("ActionButton1Cooldown:"..(self and self:GetName() or 'nil'))
